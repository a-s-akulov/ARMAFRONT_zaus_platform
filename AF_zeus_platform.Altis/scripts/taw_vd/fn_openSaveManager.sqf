...
for "_i" from 0 to 9 do {
	...
	if(!isNil "_varData") then {
		...
		_saveList lbAdd format["%1 %2",localize "STR_chvd_saveslot",_i];
		_saveList lbSetData [(lbSize _saveList)-1,"false"];
	};
};
ctrlSetText [295011,localize "STR_chvd_save"];
ctrlSetText [295012,localize "STR_chvd_hide"];
ctrlSetText [295013,localize "STR_chvd_saveManager"];
ctrlSetText [SLOT_NAME,localize "STR_chvd_SaveName"];
